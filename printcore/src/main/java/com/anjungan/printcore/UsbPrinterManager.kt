...
class UsbPrinterManager(
    ...
    private val vendorId: Int = DEFAULT_VID,
    private val productId: Int = DEFAULT_PID
) : PrinterTransport {

    companion object {
        private const val TAG = "UsbPrinter"
        const val DEFAULT_VID = 19275
        const val DEFAULT_PID = 14384
    ...
    fun findPrinter(): UsbDevice? {
        ...
        return devices.firstOrNull { dev ->
            (0 until dev.interfaceCount).any { i ->
                val cls = dev.getInterface(i).interfaceClass
                cls == UsbConstants.USB_CLASS_PRINTER ||
    ...
    /**
     * Cetak di thread background; bila tulis gagal (printer sibuk/putus),
     * koneksi ulang lalu coba sekali lagi. Callback dipanggil di thread utama.
     */
